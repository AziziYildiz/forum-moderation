...
func uploadHandler(w http.ResponseWriter, r *http.Request) {
	...
	result, err := utils.Db.Exec("INSERT INTO posts (title, image_url, author_id, content, status) VALUES (?, ?, ?, ?, 'pending')", title, filePath, userID, content)
	if err != nil {
...
func viewPostHandler(w http.ResponseWriter, r *http.Request) {
	...
	comments, err := fetchCommentsByID(postID, userID, isModerator)
	if err != nil {
...
func fetchCommentsByID(postID int, currentUserID int, isModerator bool) ([]Comment, error) {
	...
	for rows.Next() {
		...
		comment.CanDelete = (comment.AuthorID == currentUserID || isModerator) // Yorumu silme yetkisini kontrol et
...
func HomePage(w http.ResponseWriter, r *http.Request) {
	...
	loggedIn := utils.CheckLoginStatus(r)
	isModerator := false // Varsayılan olarak false.
	if loggedIn {
		userID, err := utils.GetUserIDFromCookie(r)
		if err == nil {
			isModerator, _ = utils.IsModerator(userID, utils.Db) // Moderatör olup olmadığını kontrol et.
		}
	}
