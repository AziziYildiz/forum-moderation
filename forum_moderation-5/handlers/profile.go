...
func profileHandler(w http.ResponseWriter, r *http.Request) {
	...
	err = utils.Db.QueryRow("SELECT username, email, name, about, usericon_url, created_at, role  FROM users WHERE id = ?", userID).Scan(&username, &email, &name, &about, &userIcon, &createdAt, &role)
	if err != nil {
	...
	posts, err := fetchPosts(userID, "p.author_id = ? AND p.status = 'approved'")
	if err != nil {
	...
	var pendingPosts, rejectedPosts []Post

	if isAdmin || isModerator {
		// Moderatör veya admin için tüm bekleyen ve reddedilen postlar
		pendingPosts, err = fetchPosts(0, "p.status = 'pending'")
		if err != nil {
		...
		rejectedPosts, err = fetchPosts(0, "p.status = 'rejected'")
		if err != nil {
			log.Printf("Error fetching rejected posts: %v", err)
			http.Error(w, "Error fetching rejected posts", http.StatusInternalServerError)
			return
		}
	} else {
		// Normal kullanıcı için sadece kendi bekleyen ve reddedilen postları
		pendingPosts, err = fetchPosts(userID, "p.author_id = ? AND p.status = 'pending'")
		if err != nil {
			log.Printf("Error fetching pending posts: %v", err)
			http.Error(w, "Error fetching pending posts", http.StatusInternalServerError)
			return
		...
		rejectedPosts, err = fetchPosts(userID, "p.author_id = ? AND p.status = 'rejected'")
		if err != nil {
			log.Printf("Error fetching rejected posts: %v", err)
			http.Error(w, "Error fetching rejected posts", http.StatusInternalServerError)
			return
...
func fetchPosts(userID int, condition string) ([]Post, error) {
	...
	for rows.Next() {
		var post Post
		if err := rows.Scan(&post.ID, &post.Title, &post.ImageURL, &post.AuthorName, &post.AuthorIcon, &post.CreatedAt, &post.Likes, &post.Dislikes, &post.CommentCount, &post.Category, &post.ViewCount, &post.Status); err != nil {
			return nil, err
		}
